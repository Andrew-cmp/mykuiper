...
//初始化，为weight count channel height width大小的weight申请空间
void ParamLayer::InitWeightParam(uint32_t param_count, uint32_t param_channel, uint32_t param_height,
                     uint32_t param_width){
    this->weights_ = std::vector<sftensor>(param_count);
    for(uint32_t i=0;i < param_count;i++){
        weights_.at(i) = std::make_shared<ftensor>(param_channel,param_height,param_width);
    }
}

void ParamLayer::InitBiasParam(uint32_t param_count, uint32_t param_channel, uint32_t param_height,
                   uint32_t param_width){
    this->bias_ = std::vector<sftensor>(param_count);
    for(uint32_t i=0;i < param_count;i++){
        bias_.at(i) = std::make_shared<ftensor>(param_channel,param_height,param_width);
    }                    
}
...
const std::shared_ptr<Tensor<float>> ParamLayer::weight(int32_t index) const{
