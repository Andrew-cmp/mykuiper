...
{
    CHECK(!this->data.empty()) << "Your data area of the tensor is empty.";
...
template <typename T>
inline void Tensor<T>::Transpose(std::vector<uint32_t> &shape)
{
}
template <typename T>
...
template <typename T>
void Tensor<T>::Reshape(std::vector<uint32_t> &shapes, bool row_major)
{
      CHECK(!this->data_.empty()) << "The data area of the tensor is empty.";
      CHECK(!shapes.empty());
